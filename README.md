...
.pinkBtn{
  ...
  /* ===== VALENTINE CINEMATIC (HAPPY) ===== */
.valHero{
  position:relative;
  min-height:100%;
  padding:18px 0 40px;
  overflow:hidden;
  background: radial-gradient(circle at 30% 20%, rgba(255,77,166,.18), transparent 55%),
              radial-gradient(circle at 80% 30%, rgba(58,160,255,.12), transparent 60%),
              radial-gradient(circle at 50% 80%, rgba(255,255,255,.06), transparent 60%),
              #050505;
}
.valGlow{
  ...
  background:
    ...
    radial-gradient(circle at 85% 25%, rgba(58,160,255,.12), transparent 58%),
    radial-gradient(circle at 50% 80%, rgba(255,255,255,.06), transparent 60%);
  filter: blur(16px) saturate(1.2);
  animation: valGlowMove 8s ease-in-out infinite;
  pointer-events:none;
  z-index:0;
}
@keyframes valGlowMove{
  0%,100%{transform:translateY(0) scale(1);}
...
.valStars{
  ...
  background:
    ...
    radial-gradient(circle at 55% 55%, rgba(255,255,255,.08), transparent 50%);
  opacity:.85;
  animation: valStarsDrift 10s ease-in-out infinite;
  pointer-events:none;
  z-index:1;
}
@keyframes valStarsDrift{
  0%,100%{transform:translateY(0);}
  50%{transform:translateY(10px);}
}
...
.valCardBig{
  ...
  background:
    radial-gradient(circle at 50% 10%, rgba(255,77,166,.12), transparent 60%),
    radial-gradient(circle at 70% 80%, rgba(58,160,255,.08), transparent 55%),
    rgba(15,15,15,.92);
  box-shadow:0 30px 80px rgba(0,0,0,.55);
  padding:18px 16px 16px;
...
/* floating particles for Valentine */
.valParticle{
  position:fixed;
  top:-30px;
  z-index:9999;
  ...
  opacity:.9;
  font-size:18px;
  animation: valFall linear forwards;
  white-space:nowrap;
}
@keyframes valFall{
  from{transform:translateY(-40px) translateX(0) rotate(0deg);}
  to{transform:translateY(110vh) translateX(var(--dx)) rotate(360deg);}
}
</style>
</head>
...
<div id="optionsPage" class="page scrollPage">
  ...
  <div class="card" style="text-align:center;">
    ...
    <div style="display:flex;flex-direction:column;gap:12px;max-width:420px;margin:0 auto;">
      ...
      <button onclick="openValentine()">Valentine’s Day 💝</button>
    </div>
...
<!-- VALENTINE CINEMATIC PAGE -->
<div id="valentinePage" class="page scrollPage">
  <div class="valHero" id="valHero">
    <div class="valGlow"></div>
    <div class="valStars"></div>

    <div class="valCardBig">
      <h2 class="valMainTitle">Happy Valentine’s Day, Shuttumani 💗</h2>
      <p class="valMainSub">Smile a little… because this page is only for you. 😊</p>

      <div class="valSceneBox" id="valSceneBox">
        <div class="valSceneTitle" id="valSceneTitle"> </div>
        <div class="valSceneText" id="valSceneText"> </div>
      </div>

      <div class="valBtnsRow">
        <button class="btnDark" onclick="closeValentine()">Back 🌙</button>
        <button onclick="valPrev()">◀ Prev</button>
        <button onclick="valNext()">Next ▶</button>
        <button onclick="valMagic()">Magic ✨</button>
      </div>

      <p class="valHintSmall">Tip: Tap “Magic ✨” anytime 🌹🦋💗</p>
    </div>
  </div>
</div>
...
const OPTIONS_PAGE_ID = "optionsPage";

let valIndex = 0;
let valTypingTimer = null;
let valAutoTimer = null;
let valActive = false;
...
But after you…
even simple moments feel special. 😊

Today I just want you to smile…
because this page is only for you. 💗`
  },
  {
    title: "👀 First time I saw you",
    text:
`8th standard… first time going for tuition…

I still remember that moment…
I saw you… and something inside me quietly changed.

I didn’t know you would become
my favorite person one day. ✨`
  },
  {
    title: "🌸 Days that made me fall",
    text:
`Then days passed…
and tuition became my “happy place”.
...
Small talks…
small smiles…
and my heart started choosing you more and more. 💞`
  },
  {
    title: "💌 February… I told you",
    text:
`In February… I told you how I feel.
...
the day my heart felt like it found “home”.
...
And I’m thankful…
for every day I get to see you,
talk to you,
and feel you in my life.

Happy Valentine’s Day, Shuttumani 💝`
  },
  {
    title: "♾️ Forever (the happy promise)",
    text:
`People say Valentine is one day…

But my love is not one day.
...
I want a life with you. ❤️

(Press “Magic ✨” now 😁)`
  }
];

// If your existing show(pageId) exists, we will NOT break it.
// We only add helper open/close functions.
function openValentine(){
  if(typeof show === "function") show("valentinePage");
  startValentine();
}

function closeValentine(){
  stopValentine();
  if(typeof show === "function") show(OPTIONS_PAGE_ID);
}

// controls
function valNext(){
  if(valIndex < valScenes.length - 1) valIndex++;
  renderValScene();
}
function valPrev(){
  if(valIndex > 0) valIndex--;
  renderValScene();
}

// typing effect
function renderValScene(){
  const t = document.getElementById("valSceneTitle");
  const p = document.getElementById("valSceneText");
  if(!t || !p) return;

  // stop previous typing
  if(valTypingTimer) clearInterval(valTypingTimer);

  t.textContent = valScenes[valIndex].title;
  p.textContent = "";

  const full = valScenes[valIndex].text;
  let i = 0;

  valTypingTimer = setInterval(() => {
    if(!valActive){ clearInterval(valTypingTimer); return; }
    p.textContent += full[i] || "";
    i++;
    if(i >= full.length) clearInterval(valTypingTimer);
  }, 18);
}
...
function startValentine(){
  ...
  // gentle auto-next (optional)
  if(valAutoTimer) clearInterval(valAutoTimer);
  valAutoTimer = setInterval(() => {
    if(!valActive) return;
    // only auto advance if not last
    if(valIndex < valScenes.length - 1){
      valIndex++;
      renderValScene();
      valMagic(); // small happy burst on each scene
    }
  }, 12000);
}
